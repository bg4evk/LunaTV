...
// 2. 签名覆盖 username，服务端校验签名匹配即可（时间戳仅用于展示/审计，非防重放的唯一手段）
// 3. httpOnly 需保持 false：客户端依赖从 cookie 读取 username/role（PWA）
export async function generateAuthCookie(
  ...
  if (username && process.env.PASSWORD) {
    ...
    // 使用密码作为密钥对用户名进行签名
    authData.signature = await generateSignature(username, process.env.PASSWORD);
